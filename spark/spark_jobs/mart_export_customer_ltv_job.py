...
df_customer_ltv = spark.read \
    .format("jdbc") \
    .option("url", gp_url) \
    .option("user", gp_user) \
    .option("password", gp_pass) \
    .option("dbtable", "mart.customer_ltv") \
    .load()
    
    
df_customer_ltv_today = df_customer_ltv.filter(F.col("load_date") == F.lit(p_date))   
OUT = f"s3a://{bucket}/mart/customer_ltv/load_date={p_date}"

(
    df_customer_ltv_today
    .repartition(1)
    .write
    .mode("overwrite")
    .parquet(OUT)
