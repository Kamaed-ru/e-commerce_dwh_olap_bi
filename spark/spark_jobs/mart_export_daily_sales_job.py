...
df_daily_sales = spark.read \
    .format("jdbc") \
    .option("url", gp_url) \
    .option("user", gp_user) \
    .option("password", gp_pass) \
    .option("dbtable", "mart.daily_sales") \
    .load()
    
    
df_daily_sales_today = df_daily_sales.filter(F.col("load_date") == F.lit(p_date))   

OUT = f"s3a://{bucket}/mart/daily_sales/load_date={p_date}"

(
    df_daily_sales_today
    .repartition(1)
    .write
    .mode("overwrite")
    .parquet(OUT)
